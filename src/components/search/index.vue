  ...
  <div>
    ...
    <div class="container container-middle">
      <div class="search">
        ...
        <div class="search-his" v-show="$store.state.searchHis.length > 0">
          <h3>搜索历史</h3>
          <ul class="search-list">
            <li class="item"  v-for="item in $store.state.searchHis" @click="onSearch(item)">
              <span>{{item}}</span>
            </li>
          </ul>
        </div>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created(){
      this._getSearchHot();
      this.$store.commit('GetSearchHistoryIn');
      this.hisList=this.$store.state.searchHis;
    },
    methods: {
      //热门搜索
      _getSearchHot(){
